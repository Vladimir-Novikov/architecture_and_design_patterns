from trinity_framework.main import Framework, FakeFramework, LogFramework
...
def create_wsgi_server():
    server_type = input("Выберите сервер для запуска: 1 - Основной сервер, 2 - ЛОГ_сервер, 3 - Фейковый сервер ")
    if server_type == "2":
        ...
        with make_server("", 8888, fake_wsgi) as httpd:
            print("Фейковый сервер работает на порту 8888...")
            httpd.serve_forever()
    else:
